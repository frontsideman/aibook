...
describe('ChildProfileCard', () => {
  ...
  it('renders initials for multi-word names', () => {
    render(<ChildProfileCard profile={{ ...baseProfile, name: 'Mary Jane' }} />);
  ...
  it('hides the Interests block when there are no interests', () => {
    render(<ChildProfileCard profile={{ ...baseProfile, interests: [] }} />);
  ...
  it('invokes onEdit and onDelete handlers', () => {
    ...
    render(<ChildProfileCard profile={baseProfile} onEdit={onEdit} onDelete={onDelete} />);
  ...
  it('exposes accessible aria labels for actions', () => {
    ...
    expect(screen.getByRole('button', { name: 'Edit Noah' })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: 'Delete Noah' })).toBeInTheDocument();
  });
