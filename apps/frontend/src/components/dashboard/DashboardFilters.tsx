...
export default function DashboardFilters({
  ...
  return (
    <div className="mb-5 flex flex-wrap items-center gap-2 rounded-2xl border border-border bg-surface p-2">
      ...
      <Select value={status || '__all'} onValueChange={(value) => onStatusChange(value === '__all' ? '' : value)}>
        <SelectTrigger aria-label="Filter by status" className="h-[42px] w-auto min-w-[100px]"><SelectValue placeholder="Status" /></SelectTrigger>
        <SelectContent>
          {STATUSES.map((entry) => (
      ...
      <Select value={style || '__all'} onValueChange={(value) => onStyleChange(value === '__all' ? '' : value)}>
        <SelectTrigger aria-label="Filter by style" className="h-[42px] w-auto min-w-[100px]"><SelectValue placeholder="Style" /></SelectTrigger>
        <SelectContent>
          {STYLES.map((entry) => (
      ...
      <Select value={type || '__all'} onValueChange={(value) => onTypeChange(value === '__all' ? '' : value)}>
        <SelectTrigger aria-label="Filter by type" className="h-[42px] w-auto min-w-[100px]"><SelectValue placeholder="Type" /></SelectTrigger>
        <SelectContent>
          {TYPES.map((entry) => (
      ...
      <Select value={profile || '__all'} onValueChange={(value) => onProfileChange(value === '__all' ? '' : value)}>
        <SelectTrigger aria-label="Filter by profile" className="h-[42px] w-auto min-w-[100px]"><SelectValue placeholder="Profile" /></SelectTrigger>
        <SelectContent>
      ...
      <Select value={sort} onValueChange={(value) => onSortChange(value as 'updated' | 'title')}>
        <SelectTrigger aria-label="Sort books" className="h-[42px] w-auto min-w-[100px]"><SelectValue /></SelectTrigger>
        <SelectContent>
